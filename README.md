...
Each daily job discovers pages only on curated seller domains and checks the destination's `robots.txt` before retrieving it. Expanded scans keep five Tavily queries: exact-reference, an unworn/new asking-price query, a pinned `site:` search for the extractable grey dealer (DavidSW), and two rotated remaining dealers. Retrieved URLs are round-robined across domains with extractable grey dealers first; after three failed fetches, empty extractions, or thrown fetch errors on one host the rest of that host is skipped so a 403ing marketplace cannot consume the page budget. Known JavaScript-only catalogs such as Jomashop are not fetched. Phase 1A accepts structured Product/Offer candidates only, keeps year and warranty unknown, and does not call an LLM, fetch detail pages, or convert currencies. Phase 1B treats each structured Product row as a candidate, fetches a robots-permitted detail page when condition or another required scope attribute is still unknown, grounds retained values against the row/detail source text, normalizes currency to USD, and classifies scope as in-scope, out-of-scope, or uncertain. Unknown required details carry a 0.5 weight rather than being guessed. Schema.org `NewCondition` / `UsedCondition` (and refurbished/damaged) count as grey vs resell evidence. Jomashop product pages are a JavaScript shell with no extractable HTML, so that domain is retained in the seller seed for search-domain filtering and shown as coverage-limited on **Coverage** rather than treated as a live grey feed.
...
The dashboard separates **Avg asking (grey)** (unworn) from **Avg asking (resell)** (pre-owned), applies IQR outlier removal to confirmed listings, then computes a weighted median that includes uncertain listings at half weight. A zero-IQR cluster retains its nearby observed prices rather than treating every non-identical ask as an outlier. Provisional readings display a clearly labeled approximate asking price for context; withheld readings remain **Unverified ask** and their raw medians are available in the detail/evidence view. A price is comparable only with at least three confirmed listings and no larger uncertain pool; otherwise it cannot trigger an alert, mover, moving average, or retail-premium/discount claim. Fresh zero- or thin-listing results need coverage, not a freshness refresh. Asking prices are not transaction prices.

Every retained listing must ground either the tracked full reference or Rolex's base reference when a dealer omits the `-000x` suffix. For letter-suffixed references, a dealer's bare numeric stem is accepted when the listing does not name a conflicting suffix variant. Optional comma-separated **required listing identity terms** (for example, `factory baguette`) further constrain a specific factory configuration. Listings marked out of stock, identified as watch parts or accessories, or priced below 20% of the saved retail price are retained for review but excluded from current market metrics. These terms are market-research scope, not personal notes; changing them reclassifies current rows on the next scan while preserving append-only history.
...
Pull requests to `main` run two workflows. `.github/workflows/ci.yml` posts **typecheck** (`next typegen` then `tsc --noEmit`, via `npm run typecheck`). `.github/workflows/security-scans.yml` posts **Semgrep SAST** and **Trivy Dependency & Misconfig Scan**. Trivy uses the action’s built-in workspace cache. After a job has completed once, the Main ruleset can pin that exact name to source **GitHub Actions**. Add repository secret `SEMGREP_APP_TOKEN` so the Semgrep job can authenticate; without it `semgrep ci` fails. `patchhog/security` is posted by Patchhog, not Actions.
