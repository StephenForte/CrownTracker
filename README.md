...
The dashboard separates **Avg asking (grey)** (unworn) from **Avg asking (resell)** (pre-owned), applies IQR outlier removal to confirmed listings, then computes a weighted median that includes uncertain listings at half weight. A zero-IQR cluster retains its nearby observed prices rather than treating every non-identical ask as an outlier. Provisional and withheld estimates still store and display that median as an indicative asking-price reading, but a price is comparable only with at least three confirmed listings and no larger uncertain pool; otherwise it cannot trigger an alert, mover, moving average, or retail-premium/discount claim. Asking prices are not transaction prices.
