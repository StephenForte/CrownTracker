...
Each daily job discovers pages only on curated seller domains and checks the destination's `robots.txt` before retrieving it. Expanded scans keep five Tavily queries: exact-reference, an unworn/new asking-price query, pinned `site:` searches for the grey dealers (DavidSW and Jomashop), and one rotated remaining dealer. Retrieved URLs are round-robined across domains with grey dealers first; after three failed fetches on one host the rest of that host is skipped so a 403ing marketplace cannot consume the page budget. Phase 1A accepts structured Product/Offer candidates only, keeps year and warranty unknown, and does not call an LLM, fetch detail pages, or convert currencies. Phase 1B treats each structured Product row as a candidate, fetches a robots-permitted detail page when condition or another required scope attribute is still unknown, grounds retained values against the row/detail source text, normalizes currency to USD, and classifies scope as in-scope, out-of-scope, or uncertain. Unknown required details carry a 0.5 weight rather than being guessed. Schema.org `NewCondition` / `UsedCondition` (and refurbished/damaged) count as grey vs resell evidence. Jomashop product pages are a JavaScript shell with no extractable HTML, so that domain is retained for discovery and shown as coverage-limited on **Coverage** rather than treated as a live grey feed.
