...
The dashboard separates **Avg asking (grey)** (unworn) from **Avg asking (resell)** (pre-owned), applies IQR outlier removal before a confirmed-listing median, and surfaces confidence, staleness, evidence, availability, and curated-seller trust. A zero-IQR cluster retains its nearby observed prices rather than treating every non-identical ask as an outlier. An uncertain row is retained as evidence but never sets the headline price. A price is comparable only with at least three confirmed listings and no larger uncertain pool; otherwise it is labeled provisional or withheld and cannot trigger an alert, mover, moving average, or retail-premium/discount claim. Asking prices are not transaction prices.

Every retained listing must ground either the tracked full reference or Rolex's base reference when a dealer omits the `-000x` suffix. For letter-suffixed references, a dealer's bare numeric stem is accepted only when saved identity terms independently identify the variant and the listing does not name a conflicting suffix variant. Listings marked out of stock, identified as watch parts or accessories, or priced below 20% of the saved retail price are retained for review but excluded from current market metrics. A watch can also have optional comma-separated **required listing identity terms** (for example, `factory baguette`) to constrain a specific factory configuration. These terms are market-research scope, not personal notes; changing them reclassifies current rows on the next scan while preserving append-only history.
