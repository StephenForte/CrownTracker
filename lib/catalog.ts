...
const catalog: Record<string, Omit<WatchDraft, "referenceNumber">> = {
  ...
  "126610LN": { modelName: "Oyster Perpetual Submariner Date", nickname: "Black Sub Date", retailPriceUsd: 11100, discontinued: false, photoSourceUrl: null, specs: { caseSizeMm: 41, dial: "Black", bezel: "Black Cerachrom", bracelet: "Oyster", movement: "Calibre 3235", material: "Oystersteel" }, source: { name: "Rolex", url: official, note: "Official Rolex product information; confirm current MSRP before purchase." } },
...
export function searchCatalog(query: string): CatalogSuggestion[] {
  ...
  return Object.entries(catalog)
    .map(([referenceNumber, watch]) => ({ referenceNumber, modelName: watch.modelName, nickname: watch.nickname }))
    .filter((watch) => !needle || [watch.referenceNumber, watch.modelName, watch.nickname].some((value) => value.toLowerCase().includes(needle)))
    .slice(0, 8);
}
